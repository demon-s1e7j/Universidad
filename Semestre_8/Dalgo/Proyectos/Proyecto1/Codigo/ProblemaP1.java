  ...
  public static int calculateUp(int[][]matrix, int lines, int column, int to, AccesibleOperations function) {
    ...
    for (int x = 0; x < to; x++) {
      for (int y = 0; y < column; y++){
        if (function.isAccesible(column, x, y)) {
          boolean[] values = getCeldasConInfluenciaUp(function, column, lines, x, y);
          int max = 0;
          int nMaldicion = 0;
          for (int i = 0; i < 3; i++) {
            if (values[i]) {
    ...
    return removeBestWayUp(val, matrix, function);
  }
